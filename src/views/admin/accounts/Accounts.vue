...
<template>
    <div>
        <div class="datatable-page-header">
            <div>
                <h2>{{ t('common.titles.manage', { entity: t('entity.accounts') }) }}</h2>
                <p>{{ t('common.subtitles.manage', { entity: t('entity.accounts').toLowerCase() }) }}</p>
            </div>
            <div class="header-actions">
                ...
                <Button v-tooltip.top="t('common.tooltips.add', { entity: t('entity.account') })" :label="'+ ' + t('common.labels.new') + ' ' + t('entity.account')" severity="primary" @click="addRecord" />
            </div>
        </div>
        <DataTable
            ref="recordDataTable"
