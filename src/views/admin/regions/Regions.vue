...
// Filters configuration for regions
...
const openDialog = () => {
    dialog.open(formComponent, {
        props: {
            header: t('common.titles.add', { entity: t('entity.region') }),
            style: { width: '40vw' },
            ...
            if (result && result.data?.record?.id) {
                switch (result.data?.action) {
                    case ACTIONS.CREATE:
                        ...
                        showToast('success', ACTIONS.CREATE, 'region', 'tc');
                        break;
                    case ACTIONS.EDIT: {
                        ...
                        showToast('success', ACTIONS.EDIT, 'region', 'tc');
                        break;
                    }
                    default:
        ...
        accept: () => {
            useRegionService
                ...
                .catch((error) => {
                    ...
                    console.error('Error deleting regions');
                });
        }
    });
}
...
<template>
    <div>
        <div class="datatable-page-header">
            <div>
                <h2>{{ t('common.titles.manage', { entity: t('entity.regions') }) }}</h2>
                <p>{{ t('common.subtitles.manage', { entity: t('entity.regions').toLowerCase() }) }}</p>
            </div>
            <div class="header-actions">
                ...
                <Button v-tooltip.top="t('common.tooltips.add', { entity: t('entity.region') })" :label="'+ ' + t('common.labels.new') + ' ' + t('entity.region')" severity="primary" @click="addRecord" />
            </div>
        </div>
        <DataTable
            ref="recordDataTable"
            ...
            :currentPageReportTemplate="t('common.paggination.showing_to_of_entity', { first: '{first}', last: '{last}', totalRecords: '{totalRecords}', entity: t('entity.region') })"
            resizableColumns
        ...
        >
            ...
            >
                <template #header>
                    <HeaderCell
                        :text="t('region.columns.code')"
                        :frozen="frozenColumns.code"
                        :reorderTooltip="t('common.tooltips.reorder_columns')"
                        :lockTooltip="t('common.tooltips.lock_column')"
                        :unlockTooltip="t('common.tooltips.unlock_column')"
                        @toggle="toggleColumnFrozen('code')"
                    />
                </template>
                <template #body="{ data }">
                    <DataCell>
                        <div :class="{ 'font-bold': frozenColumns.code }">{{ data.code }}</div>
                    </DataCell>
                </template>
                <template #filter="{ filterModel, applyFilter }">
                    <InputGroup>
            ...
            >
                <template #header>
                    <HeaderCell
                        :text="t('region.columns.created_at')"
                        :frozen="frozenColumns.created_at"
                        :reorderTooltip="t('common.tooltips.reorder_columns')"
                        :lockTooltip="t('common.tooltips.lock_column')"
                        :unlockTooltip="t('common.tooltips.unlock_column')"
            ...
            >
                <template #header>
                    <HeaderCell
                        :text="t('region.columns.updated_at')"
                        :frozen="frozenColumns.updated_at"
                        :reorderTooltip="t('common.tooltips.reorder_columns')"
                        :lockTooltip="t('common.tooltips.lock_column')"
                        :unlockTooltip="t('common.tooltips.unlock_column')"
                ...
                <template #body="{ data, frozenRow, index }">
                    <DataCell>
                        <div class="flex items-center justify-center gap-1">
                            <RowActionMenu
                                :actions="[
                                    { label: t('common.labels.view'), icon: 'pi pi-eye', command: () => editRecord(data), visible: authStore.hasPermission('view_regions') },
                                    { label: t('common.labels.edit'), icon: 'pi pi-pencil', command: () => editRecord(data), visible: authStore.hasPermission('update_regions') },
                                    { label: t('common.labels.delete'), icon: 'pi pi-trash', severity: 'danger', command: () => confirmDeleteRecord(null, [data.id]), visible: authStore.hasPermission('delete_regions') }
                                ]"
                            />
                            <Button
