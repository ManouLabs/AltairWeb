...
function getReputation(customerId: number): ReputationData {
    if (!customerId) return { level: 'new', label: t('customer.reputation.levels.new'), percentage: null, delivered: 0, returned: 0, color: '#64748b', bgColor: '#f1f5f9' };
    const fakeReputations: ReputationData[] = [
        { level: 'excellent', label: t('customer.reputation.levels.excellent'), percentage: 95, delivered: 47, returned: 3, color: '#10b981', bgColor: '#d1fae5' },
...
async function fetchShippingFee(): Promise<void> {
    ...
    try {
        ...
        record.value.shipping_fee = response.shipping_fee;
    } catch (error) {
            ...
            <div class="lg:col-span-2 flex flex-col gap-6">
                ...
                <div class="bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-xl p-6 shadow-sm">
                        ...
                        >
                            <template #option="{ option }">
                                <div class="flex items-center gap-3 py-1">
                                    <div class="flex items-center justify-center w-8 h-8 rounded-full bg-primary/10 text-primary text-xs font-bold">
                                        {{ getCustomerInitials(option.name) }}
                                    </div>
                                    <div>
                                        <div class="font-semibold text-sm">{{ option.name }}</div>
                                        <div class="text-xs text-surface-400">{{ getCustomerDetails(option).phone }}</div>
                                    </div>
                                </div>
                            </template>
                        </AutoComplete>
                    ...
                    <div v-if="selectedCustomer && selectedCustomer.id" class="p-4 rounded-xl border border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-800 mb-8">
                        ...
                        <div class="flex items-center justify-between mb-3">
                            <div class="flex items-center gap-3">
                                <div class="relative">
                                    <div class="flex items-center justify-center w-11 h-11 rounded-full bg-primary text-white text-sm font-bold">
                        ...
                        <div class="flex items-start gap-6">
                            ...
                            <div class="w-1/2">
                                <div class="flex items-center justify-between mb-1">
                                    <span class="text-xs font-bold uppercase tracking-wider" :style="{ color: getReputation(selectedCustomer.id).color }">
                                        {{ getReputation(selectedCustomer.id).label }}
                                        <template v-if="getReputation(selectedCustomer.id).percentage !== null">({{ getReputation(selectedCustomer.id).percentage }}%)</template>
                                    </span>
                                </div>
                                <div class="w-full h-2 rounded-full overflow-hidden" :style="{ backgroundColor: getReputation(selectedCustomer.id).bgColor }">
                                    <div class="h-full rounded-full transition-all duration-500" :style="{ width: (getReputation(selectedCustomer.id).percentage ?? 0) + '%', backgroundColor: getReputation(selectedCustomer.id).color }"></div>
                                </div>
                                <div v-if="getReputation(selectedCustomer.id).level !== 'new'" class="flex items-center gap-3 mt-1.5 text-xs">
                                    <span class="flex items-center gap-1 text-green-600"><i class="pi pi-check-circle text-[10px]"></i> {{ getReputation(selectedCustomer.id).delivered }}</span>
                                    <span class="flex items-center gap-1 text-red-500"><i class="pi pi-times-circle text-[10px]"></i> {{ getReputation(selectedCustomer.id).returned }}</span>
                                </div>
                                <div v-else class="text-xs text-surface-400 mt-1.5 italic">{{ t('customer.reputation.labels.no_history') }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div>
                            <div class="flex gap-2">
                                <div
                                    v-for="opt in shippingTypeOptions"
                                    :key="opt.value"
                                    class="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all duration-200"
                                    :class="record.shipping_type === opt.value ? 'border-primary bg-primary/5 dark:bg-primary/10 shadow-sm' : 'border-surface-200 dark:border-surface-700 hover:border-surface-300'"
                                    @click="record.shipping_type = opt.value"
                                >
                                    <i :class="opt.icon" class="text-base" :style="{ color: record.shipping_type === opt.value ? 'var(--p-primary-color)' : 'var(--p-text-muted-color)' }" />
                                    <span class="text-xs font-bold uppercase tracking-wider" :class="record.shipping_type === opt.value ? 'text-primary' : 'text-surface-400'">{{ opt.label }}</span>
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-xl p-6 shadow-sm">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <button type="button" class="flex items-center gap-1.5 text-sm text-primary hover:text-primary/80 font-medium cursor-pointer transition-colors" @click="addCustomItem">
                            <i class="pi pi-plus-circle text-sm"></i>
                            {{ t('order.labels.add_custom_item') }}
                        </button>
                    </div>
                    ...
                    <div class="mb-4">
                        <div class="flex items-center border-2 border-primary/30 rounded-xl overflow-hidden focus-within:border-primary transition-colors">
                            ...
                            >
                                <template #option="{ option }">
                                    <div class="flex items-center gap-3 py-1">
                                        <div class="w-8 h-8 rounded bg-surface-100 dark:bg-surface-700 flex items-center justify-center">
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="border-b border-surface-200 dark:border-surface-700">
                                    <th class="text-left py-3 px-2 font-semibold text-surface-500 dark:text-surface-400 uppercase text-xs tracking-wider w-8">#</th>
                                    ...
                                    <th class="text-left py-3 px-2 font-semibold text-surface-500 dark:text-surface-400 uppercase text-xs tracking-wider">{{ t('order.columns.sku') }}</th>
                                    <th class="text-center py-3 px-2 font-semibold text-surface-500 dark:text-surface-400 uppercase text-xs tracking-wider">{{ t('order.columns.qty') }}</th>
                                    <th class="text-right py-3 px-2 font-semibold text-surface-500 dark:text-surface-400 uppercase text-xs tracking-wider">{{ t('order.columns.price') }}</th>
                                    <th class="text-right py-3 px-2 font-semibold text-surface-500 dark:text-surface-400 uppercase text-xs tracking-wider">{{ t('order.columns.item_total') }}</th>
                                    <th class="w-10"></th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(item, index) in record.items" :key="index" class="border-b border-surface-100 dark:border-surface-800">
                                    ...
                                    <td class="py-4 px-2">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="font-semibold text-surface-800 dark:text-surface-100">{{ item.product_name || 'Custom Item' }}</div>
                ...
                <div class="bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-xl p-6 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <!-- Payment Status -->
                        <div>
                            <label class="text-sm font-medium text-surface-700 dark:text-surface-200 mb-2 block">{{ t('order.columns.payment_status') }}</label>
                            <div class="flex gap-2">
                                <div
                                    class="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all duration-200"
                                    :class="record.payment_status === 'not_paid' ? 'border-amber-500 bg-amber-50 dark:bg-amber-900/20' : 'border-surface-200 dark:border-surface-700 hover:border-surface-300'"
                                    @click="record.payment_status = 'not_paid'"
                                >
                                    <i class="pi pi-wallet text-base" :class="record.payment_status === 'not_paid' ? 'text-amber-600' : 'text-surface-400'" />
                                    <span class="text-xs font-bold uppercase" :class="record.payment_status === 'not_paid' ? 'text-amber-700 dark:text-amber-400' : 'text-surface-400'">{{ t('order.payment_statuses.not_paid') }}</span>
                                </div>
                                <div
                                    class="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all duration-200"
                                    :class="record.payment_status === 'paid' ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : 'border-surface-200 dark:border-surface-700 hover:border-surface-300'"
                                    @click="record.payment_status = 'paid'"
                                >
                                    <i class="pi pi-check-circle text-base" :class="record.payment_status === 'paid' ? 'text-green-600' : 'text-surface-400'" />
                                    <span class="text-xs font-bold uppercase" :class="record.payment_status === 'paid' ? 'text-green-700 dark:text-green-400' : 'text-surface-400'">{{ t('order.payment_statuses.paid') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
