...
const openDialog = () => {
    ...
    dialog.open(formComponent, {
        props: {
            header: isEdit ? t('common.titles.edit', { entity: t('entity.customer') }) : t('common.titles.add', { entity: t('entity.customer') }),
            style: { width: '50vw' },
...
<template>
    <div>
        <div class="datatable-page-header">
            <div>
                <h2>{{ t('common.titles.manage', { entity: t('entity.customers') }) }}</h2>
                <p>{{ t('customer.labels.manage_subtitle') }}</p>
            </div>
