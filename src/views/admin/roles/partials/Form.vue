...
function saveRecord() {
    ...
    const serviceAction = action.value === 'store' ? useRoleService.storeRole : (roleData) => useRoleService.updateRole(record.value.id, roleData);
    ...
    serviceAction(record.value)
        ...
        .catch((error) => {
            ...
            toast.add({
                severity: 'error',
                summary: t('common.toasts.error.summary'),
...
<template>
    <div class="flex flex-col gap-8 pt-2">
        <div>
            <FloatLabel variant="on">
                ...
                <InputText :disabled="loading.isPageLoading" id="name" v-model.trim="record.name" required="true" autofocus fluid :invalid="errors?.name ? true : false" />
            </FloatLabel>
    ...
    <div class="flex justify-end gap-2 mt-4">
        ...
        <Button :label="t('common.actions.save')" icon="pi pi-check" @click="saveRecord" :loading="loading.isPageLoading" />
    </div>
</template>
